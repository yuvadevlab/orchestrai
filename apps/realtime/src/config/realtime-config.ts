...
export const RealtimeConfigSchema = z.object({
  ...
  /** Secret key used to verify client JWT authentication tokens */
  jwtSecret: z.string().min(1).default("orchestrai-dev-jwt-secret-key"),
...
export function loadRealtimeConfig(
  ...
  return RealtimeConfigSchema.parse({
    ...
    jwtSecret: env.JWT_SECRET ?? "orchestrai-dev-jwt-secret-key",
